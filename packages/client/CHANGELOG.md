...
- [`064cd99`](https://github.com/modelcontextprotocol/typescript-sdk/commit/064cd990cd8f294b25a8c57b91845fda0c0b126d) Thanks [@felixweinberger](https://github.com/felixweinberger)! - Detach stdio transport stdout/stdin listeners on `close()`. Previously, if the child process
  wrote to stdout during shutdown (after `close()` was called), the still-attached data listener would attempt to parse it as JSON-RPC and emit a spurious parse error via `onerror`. Fixes #780.
